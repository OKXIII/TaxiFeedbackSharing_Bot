...
#Получение информации из БД
    def get_info_lp(self, src):
        with self.connection:
            self.cursor.execute("SELECT licenseplate.date, comment.carmodel, comment.comment, comment.driver, comment.grade FROM licenseplate JOIN comment ON licenseplate.id=comment.license_plate_id WHERE licenseplate.licenseplate='{}'".format(src))
